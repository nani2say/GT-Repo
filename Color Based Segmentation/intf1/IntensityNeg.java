...
class IntensityNeg extends Frame 
{   
	BufferedImage img2;
	int x=0;
	int y=0;
  ...
  BufferedImage applyInegative(BufferedImage img)
	{
		  try{
       ...
       hsi=new float[ih][iw][4];
		...
		for(int x=0;x<ih;x++)
			...
			//to apply negative filter I =Imax-I, H=h, S=s
				...
				for(int y=0;y<iw;y++)
				 {
				   ...
				   hsi[x][y][3]=imax- hsi[x][y][3];       //decimalformat.format(hs[2]);    
