...
class IlluminHSV implements ActionListener
{
	...
	JFrame f;
	JTextField tf,tf2,tf3; JButton button;
	 JLabel label,label1,label2;
	Intf f2;
	

	IlluminHSV(BufferedImage img3)
	{
       f2= new Intf();
	  f2.setSize(800,800);
		...
		f2.repaint();
			 ...
			 tf2 = new JTextField(10);
			  ...
			  label2= new JLabel(" Enter Intensity<0-100> ");

             button = new JButton(" Get Image ");
			 
              f.setLayout(new FlowLayout());

			 f.add(label); f.add(tf); f.add(label1); f.add(tf2); f.add(label2); f.add(tf3);f.add(button);
...
// to Find Smax
float smax=hsi[0][0][2] , imax=hsi[0][0][3];
				...
				for(int y=0;y<iw;y++)
				 {
			        ...
			        if(imax<hsi[x][y][3]) imax= hsi[x][y][3];
...
float d=0,h=0,s=0;
	 ...
	 for(int x=0;x<ih;x++)
		   ...
		   // to modify I value
		             hsi[x][y][3] = hsi[x][y][3]*intensity/imax;
 ...
 public void actionPerformed(ActionEvent ae) 
	 {
      ...
      intensity = Integer.parseInt(tf3.getText());
